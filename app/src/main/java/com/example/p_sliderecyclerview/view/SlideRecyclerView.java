package com.example.p_sliderecyclerview.view;
...
/*onInterceptTouchEvent:是定義於ViewGroup裡面的一個方法，此事件是用於攔截觸控事件的，
ViewGroup(繼承自View)，一個View的Group，也就是我們的一個佈局如LinerLayout，各個佈局類都繼承自ViewGroup；
onTouchEvent:是定義於View中的一個方法，處理傳遞到View的手勢觸控事件。
手勢事件型別包括ACTION_DOWN,ACTION_MOVE,ACTION_UP,ACTION_CANCEL等事件；
其中ViewGroup裡的onInterceptTouchEvent預設返回值是false，這樣touch事件會傳遞到View控制元件，
ViewGroup裡的onTouchEvent預設返回值是false；
View裡的onTouchEvent預設返回值是true，當我們手指點選螢幕時候，先呼叫ACTION_DOWN事件，
當onTouchEvent裡返回值是true的時候，onTouch會繼續呼叫ACTION_UP事件，如果onTouchEvent裡返回值是false，
那麼onTouchEvent只會呼叫ACTION_DOWN而不呼叫ACTION_UP。
*/
...
public class SlideRecyclerView extends RecyclerView {
    ...
    private VelocityTracker mVelocityTracker;   // 記錄滑動速度追踪器
    ...
    public SlideRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
        ...
        //它獲得的是觸發移動事件的最短距離，如果小於這個距離就不觸發移動控件，如viewpager就是用這個距離來判斷用戶是否翻頁
    ...
    public boolean onInterceptTouchEvent(MotionEvent e) {
        ...
        int y = (int) e.getY();
        obtainVelocity(e);
        Log.v(TAG,"onInterceptTouchEvent -> MotionEvent:" + e +" /x:" + x +" /y:" + y);
        switch (e.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if (!mScroller.isFinished()) {  // 如果动画还没停止，则立即终止动画
                    mScroller.abortAnimation(); //終止動畫
                    Log.v(TAG,"onInterceptTouchEvent -> isFinished:" +"mScroller.abortAnimation()");
                }
                mFirstX = mLastX = x;
                mFirstY = y;
